...
// ════════════════════════════════════════════════════
// mobile-app/src/store/auth.store.ts
// ════════════════════════════════════════════════════
...
interface AuthStore {
  user: AuthUser | null;
  token: string | null;
  setAuth: (user: AuthUser, token: string, refresh: string) => Promise<void>;
  logout: () => Promise<void>;
...
// ════════════════════════════════════════════════════
// mobile-app/src/store/cart.store.ts
// ════════════════════════════════════════════════════
import { create } from 'zustand';

interface CartItem {
  productId: string; name: string; price: number;
  size: string; imageUrl?: string; qty: number;
}
...
export const useCartStore = create<CartStore>((set, get) => ({
  ...
  addItem: (item) => {
    ...
    if (existing) {
      updated = items.map(i => i.productId === item.productId && i.size === item.size ? { ...i, qty: i.qty + 1 } : i);
    } else {
      updated = [...items, { ...item, qty: 1 }];
    }
    set({
      items: updated,
      totalItems: updated.reduce((s, i) => s + i.qty, 0),
      totalPrice: updated.reduce((s, i) => s + i.price * i.qty, 0),
    });
  },

  removeItem: (productId, size) => {
    const updated = get().items.filter(i => !(i.productId === productId && i.size === size));
    set({ items: updated, totalItems: updated.reduce((s,i) => s+i.qty, 0), totalPrice: updated.reduce((s,i) => s+i.price*i.qty, 0) });
  },

  updateQty: (productId, size, qty) => {
    const updated = qty <= 0
      ? get().items.filter(i => !(i.productId === productId && i.size === size))
      : get().items.map(i => i.productId === productId && i.size === size ? { ...i, qty } : i);
    set({ items: updated, totalItems: updated.reduce((s,i) => s+i.qty, 0), totalPrice: updated.reduce((s,i) => s+i.price*i.qty, 0) });
  },
...
export const useAuthStore = create<AuthStore>((set) => ({
  user:  JSON.parse(localStorage.getItem('spider_user') || 'null'),
  token: localStorage.getItem('spider_token'),

  setAuth: (user, token) => {
    localStorage.setItem('spider_token', token);
    localStorage.setItem('spider_user',  JSON.stringify(user));
    set({ user, token });
  },
